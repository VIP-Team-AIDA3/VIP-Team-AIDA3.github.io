...
- A pitot-static airspeed sensor reports 50.8 kt, then 49.6 kt, then 51.2 kt while the aircraft is held at a known, constant airspeed in a wind-tunnel or tow test. What is the sensor's true bias and noise level?
...
- You changed your autoland control law and mean touchdown deviation from the runway centerline dropped from 12.46 m to 11.95 m across a batch of simulated approaches. Is that a real improvement, or could it be simulation noise?
- Your Extended Kalman Filter (EKF) or Unscented Kalman Filter — the workhorse of every modern flight computer for attitude, position, and velocity estimation — is a sequential Bayesian estimator by construction, fusing IMU, GPS, magnetometer, barometer, and pitot data in real time.
...
1. **Point estimation.** Collect 15 stationary readings from any onboard sensor (barometer, magnetometer heading, GPS altitude) against a known reference. Compute the MLE (sample mean), its 95% frequentist CI, and — assuming a known noise level from the datasheet — a Bayesian posterior with a prior centered at the datasheet's nominal value. Compare the two intervals.
2. **Prior sensitivity.** Redo the Bayesian calculation from Exercise 1 with three different priors: very tight ($\sigma_0 = 0.1$), the original ($\sigma_0 = 1.0$), and very loose ($\sigma_0 = 10$). How much does the posterior mean shift? At what point does the data "win" over the prior? This is exactly the sensitivity check you'd want to run before trusting a fusion filter's prior in a safety-critical context.
3. **Sequential updating.** Implement the streaming Bayesian update from Section 6 using real or simulated airspeed/altitude data and plot posterior mean and standard deviation vs. number of observations. At what $n$ (or elapsed flight time) does the posterior standard deviation drop below a target tolerance (e.g., 0.1 kt)? This answers "how long does my filter need to converge after initialization or a GPS reacquisition?"
4. **A/B comparison of control laws.** Take two configurations of your autopilot (e.g., two gain sets for an autoland controller, or two path-planning algorithms) and run each 8–10 times in simulation. Perform both a frequentist two-sample t-test and a Bayesian posterior-difference analysis (Section 7) on touchdown deviation or path-tracking error. Do they agree on the conclusion? Which gives you more actionable information for a design review?
5. **Connect to your EKF/UKF.** Look up the 1-D Kalman filter update equations used in your team's flight-stack (PX4's EKF2, ArduPilot's EKF3, or your own implementation). Show algebraically that the measurement-update step is identical to the precision-weighted Bayesian update in Section 6, with the "prediction" step adding a process-noise term driven by the aircraft's dynamics model. This is the bridge between what you just learned and the state estimator your aircraft actually flies on.
6. **Perception confidence as a prior.** If your VIP team works on vision-based detect-and-avoid or runway detection, take a batch of your model's detection confidences and treat the prior probability of an obstacle/runway being present (from context, e.g., known traffic density or airport layout) as a Bayesian prior. Combine it with the detector's likelihood to get a posterior probability of a true detection, and compare that decision rule to a simple frequentist confidence threshold.
...
- Thrun, Burgard, Fox. *Probabilistic Robotics* — shows exactly how Bayesian inference underlies localization, mapping, and sensor fusion; the math transfers directly to UAV state estimation.
- Downey, A. *Think Bayes* — a free, code-first introduction to Bayesian statistics in Python, very close in spirit to the worked examples above.
